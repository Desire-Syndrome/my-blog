		...
		{article && !articleGetLoading ? (<>
			<div className='container px-4 2xl:px-20 mx-auto'>
						...
						<div className='mt-8 md:mt-0 md:w-2/5 md:ms-4 text-center'>
							...
							{userInfo?.isAdmin && (
								<div className='mt-5 flex justify-center gap-2'>
									<button onClick={() => articleUpdateHandler(article._id)} className='bg-sky-600 rounded px-4 py-2 text-white hover:bg-sky-500 transition duration-300 ease-in-out'>
										Update
									</button>
									<button onClick={() => { articleDeleteHandler(); setArticleToDelete(article._id); }} className='bg-rose-600 rounded px-4 py-2 text-white hover:bg-rose-500 transition duration-300 ease-in-out'>
										Delete
									</button>
								</div>
							)}
				...
				<div className='w-full md:w-5/6 m-auto mt-12'>
					...
					<div dangerouslySetInnerHTML={{ __html: article.fullText }} className='mt-8 rich-text'></div>
				</div>
			</div>
			...
			<div className='mt-12 py-10 container px-4 2xl:px-20 mx-auto'>
				<p className='text-lg sm:text-xl font-medium  text-center text-neutral-700'>
					{articleGetError}
				</p>
			</div>
		)
