...
const Dashboard = () => {
	...
	return (
			...
			<div className='flex items-start'>
				<div className='border-r border-sky-200 max-w-1/4 border-b md:border-hidden'>
					<ul className='flex flex-col items-start pt-5 pb-2 text-gray-800'>
						...
						<li className='w-full mt-2 border-t border-sky-200'><NavLink to={'/dashboard/new-article'} className={({ isActive }) => `flex items-center p-3 md:px-6 gap-2 w-full mt-2 ${isActive && 'bg-sky-200'}`} >
							<img src={assetsImages.icon5} className='w-5' alt="icon" /><p className='max-md:hidden'>New Article</p>
						</NavLink></li>
						<li className='w-full'><NavLink to={'/dashboard/my-articles'} className={({ isActive }) => `flex items-center p-3 md:px-6 gap-2 w-full ${isActive && 'bg-sky-200'}`} >
							<img src={assetsImages.icon6} className='w-5' alt="icon" /><p className='max-md:hidden'>My Articles</p>
						</NavLink></li>
						<li className='w-full'><NavLink to={'/dashboard/my-reviews'} className={({ isActive }) => `flex items-center p-3 md:px-6 gap-2 w-full ${isActive && 'bg-sky-200'}`} >
							<img src={assetsImages.icon7} className='w-5' alt="icon" /><p className='max-md:hidden'>My Reviews</p>
						</NavLink></li>
						<li className='w-full mt-2 border-t border-sky-200'><NavLink to={'/dashboard/edit-profile'} className={({ isActive }) => `flex items-center p-3 md:px-6 gap-2 w-full mt-2 ${isActive && 'bg-sky-200'}`} >
